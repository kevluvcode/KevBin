...
<div class="container" style="max-width: 900px;">
    ...
    <p class="text-secondary mb-4 reveal in-view">Everything is served through a <strong>Cloudflare Worker</strong> at <code>https://autumn-term-1e5c.kevinkevcounts1.workers.dev/api?action=...</code> as JSON with CORS enabled (<code>Access-Control-Allow-Origin: *</code>), so browsers can fetch it directly.</p>
    ...
    <div class="table-responsive reveal">
        <table class="table table-dark table-striped align-middle small" style="--bs-table-bg:#101010;">
            ...
            <tbody>
            <tr><td><code>?action=stats</code></td><td>site counters: users, pastes, links, tools, live online</td><td><code>https://autumn-term-1e5c.kevinkevcounts1.workers.dev/api?action=stats</code></td></tr>
            <tr><td><code>?action=online</code></td><td>live online count (real-time widget friendly)</td><td><code>https://autumn-term-1e5c.kevinkevcounts1.workers.dev/api?action=online</code></td></tr>
            <tr><td><code>?action=pastes</code></td><td>latest public pastes (add <code>&limit=50</code>, max 100, and <code>&offset=50</code> to page)</td><td><code>https://autumn-term-1e5c.kevinkevcounts1.workers.dev/api?action=pastes&amp;limit=50</code></td></tr>
            <tr><td><code>?action=paste&amp;id=...</code></td><td>full content of one public paste</td><td><code>https://autumn-term-1e5c.kevinkevcounts1.workers.dev/api?action=paste&amp;id=abcdef12</code></td></tr>
            <tr><td><code>?action=tools</code></td><td>all public tools (usable by anyone, no account)</td><td><code>https://autumn-term-1e5c.kevinkevcounts1.workers.dev/api?action=tools</code></td></tr>
            <tr><td><code>?action=updates</code></td><td>the public update log</td><td><code>https://autumn-term-1e5c.kevinkevcounts1.workers.dev/api?action=updates</code></td></tr>
            <tr><td><code>?action=user</code></td><td>user count only (browsing profiles is not available)</td><td><code>https://autumn-term-1e5c.kevinkevcounts1.workers.dev/api?action=user</code></td></tr>
            </tbody>
        </table>
    </div>
...
<pre class="paste-content small">curl "https://autumn-term-1e5c.kevinkevcounts1.workers.dev/api?action=stats"</pre>
    </div>

    <div class="mt-3 reveal">
        <div class="text-secondary small mb-1">JavaScript (works in any page, CORS-friendly)</div>
<pre class="paste-content small">fetch("https://autumn-term-1e5c.kevinkevcounts1.workers.dev/api?action=online")
  .then(r =&gt; r.json())
  .then(d =&gt; console.log("online: " + d.online));

fetch("https://autumn-term-1e5c.kevinkevcounts1.workers.dev/api?action=pastes&amp;limit=10")
  .then(r =&gt; r.json())
...
with urllib.request.urlopen("https://autumn-term-1e5c.kevinkevcounts1.workers.dev/api?action=stats") as r:
    data = json.load(r)
...
<pre class="paste-content small">$data = json_decode(file_get_contents("https://autumn-term-1e5c.kevinkevcounts1.workers.dev/api?action=stats"), true);
...
<pre class="paste-content small">curl -H "Content-Type: application/json" \
  -d '{"username":"KevBin Stats","content":"Live now: '"$(curl -s "https://autumn-term-1e5c.kevinkevcounts1.workers.dev/api?action=online" | grep -o '"online":[0-9]*' | cut -d: -f2)"'" users online on KevBin"}' \
...
API = "https://autumn-term-1e5c.kevinkevcounts1.workers.dev/api"
...
async def kevbin(interaction: discord.Interaction):
    ...
    <p class="text-secondary small mt-3 reveal"><strong>Option 4 — Discord slash command.</strong> Use the built-in <code>/kevbin</code> command in any server with the bot — shows live stats, latest pastes and tools directly in Discord.</p>

...
<style>
  ...
  <p class="ok">Worker is live and healthy.</p>
    ...
    if (request.method === 'POST' && url.pathname === '/oauth/exchange') {
      ...
      const secret = env.DISCORD_CLIENT_SECRET /* set via: wrangler secret put DISCORD_CLIENT_SECRET */;
    ...
    if (request.method === 'POST' && url.pathname === '/webhook-events') {
      ...
      if (body && (body.type === 1 || body.event)) {
        ...
        if (env.DISCORD_BOT_TOKEN) {
          try {
            const channelId = await getDMChannel(env.DISCORD_BOT_TOKEN, 'YOUR_USER_ID');
            if (channelId) {
